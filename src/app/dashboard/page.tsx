          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {featuredProjects.map((project) => (
                ...
                >
                  <span className={`px-3 py-1 ${project.tagColor} text-white text-sm font-medium rounded-full`}>{project.tag}</span>

                  <div className="w-full aspect-square bg-slate-700/50 rounded-xl mb-4 flex items-center justify-center overflow-hidden">
                    {project.id === 1 && <img src="/img1.png" alt="Assignment" className="w-full h-full object-contain rounded-xl" />}
                    {project.id === 2 && <img src="/img2.png" alt="Termwork" className="w-full h-full object-contain rounded-xl" />}
                    {project.id !== 1 && project.id !== 2 && <div className="w-12 h-12 bg-slate-600 rounded-lg opacity-50"></div>}
                  </div>
                  ...
                  <div className="flex items-center gap-4 mb-4 text-sm text-slate-400">
                    <div className="flex items-center gap-1">
                      <Star className="w-4 h-4 text-yellow-500 fill-current" /> {project.rating} ({project.reviews} reviews)
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (project.id === 2) router.push('/form?noOfPage=24&totalAmount=250');
                        if (project.id === 3) router.push('/form?noOfPage=8&totalAmount=50');
                      }}
                    >
                      Add to Cart
                    </button>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div>
                ...
                {categories.map((cat) => <a key={cat} href="#" className="block text-slate-400 hover:text-white transition-colors">{cat}</a>)}
              </div>
